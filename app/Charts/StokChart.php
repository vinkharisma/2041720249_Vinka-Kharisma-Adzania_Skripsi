...
class StokChart
{
    ...
    public function buildStokAkhirChart($data): \ArielMejiaDev\LarapexCharts\LineChart
    {
        $dataArray = $data->pluck('stok_akhir')->toArray();
        $labels = $data->pluck('tanggal')->toArray();

        return $this->chart->lineChart()
            // ->setTitle('Stok Akhir')
            ->addData('Stok Akhir', $dataArray)
            ->setLabels($labels)
            ->setFontFamily('Nunito, Segoe UI, Arial')
            ->setGrid();
    }

    public function buildPaletTerpakaiChart($data): \ArielMejiaDev\LarapexCharts\LineChart
    {
        // Ambil data stok akhir dan bulatkan nilainya
        $dataStokAkhir = $data->pluck('stok_akhir')->map(function($value) {
            return round($value);
        })->toArray();

        // Format tanggal menjadi 'd M Y'
        $dataTanggal = $data->pluck('tanggal')->map(function($date) {
            return \Carbon\Carbon::parse($date)->format('d M Y');
        })->toArray();

        return $this->chart->lineChart()
            // ->setTitle('Palet Terpakai')
            ->addData('Stok Akhir', $dataStokAkhir) // Data stok akhir yang sudah dibulatkan
            ->setXAxis($dataTanggal) // Format tanggal yang sudah diubah
            ->setHeight(400)
            ->setColors(['#394eea']) // Warna untuk garis stok akhir
            ->setFontColor('#6c757d')
            ->setFontFamily('Nunito, Segoe UI, Arial')
            ->setGrid();
    }
    ...
    public function buildPaletKosongChart($data): \ArielMejiaDev\LarapexCharts\LineChart
    {
        // Ambil data stok akhir dan bulatkan nilainya
        $dataStokAkhir = $data->pluck('stok_akhir')->map(function($value) {
            return round($value);
        ...
        return $this->chart->lineChart()
            // ->setTitle('Palet Kosong')
            ->addData('Stok Akhir', $dataStokAkhir) // Data stok akhir yang sudah dibulatkan
            ->setXAxis($dataTanggal) // Format tanggal yang sudah diubah
            ->setHeight(400)
            ->setColors(['#ffa426']) // Warna untuk garis stok akhir
            ->setFontColor('#6c757d')
            ->setFontFamily('Nunito, Segoe UI, Arial')
            ->setGrid();
    }

    public function buildPrediksiChart($data): \ArielMejiaDev\LarapexCharts\LineChart
    {
        // Ambil hasil prediksi dan bulatkan nilainya
        $dataArray = $data->pluck('hasil')->map(function($value) {
            return round($value);
        })->toArray();
